...
import type { GroupNewMessagePayload } from '@/hooks/useGroupSocket';
...
export default function GroupChatPage() {
  ...
  const [inviteResults, setInviteResults] = useState<Array<{ id: string; name: string | null; image: string | null; userTag: string | null }>>([]);
  ...
  const handleCopyLink = useCallback((message: MessageItem) => {
    setContextMenu(null);
    const url = `${typeof window !== 'undefined' ? window.location.origin : ''}/messages/group/${groupId}#msg-${message.id}`;
  ...
  const handleDeleteForMe = useCallback(
    async (messageId: string) => {
      if (!groupId || actioningId) return;
      setContextMenu(null);
      setActioningId(messageId);
      try {
        const res = await fetch(
          `/api/me/group-chats/${encodeURIComponent(groupId)}/messages/${encodeURIComponent(messageId)}/hide`,
  ...
  const handleDeleteForAll = useCallback(
    async (messageId: string) => {
      ...
      try {
        const res = await fetch(
          `/api/me/group-chats/${encodeURIComponent(groupId)}/messages/${encodeURIComponent(messageId)}`,
  ...
  useGroupSocket(
    groupId,
    useCallback((msg: GroupNewMessagePayload) => {
      appendMessage({
        id: msg.id,
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([
      fetch(`/api/me/group-chats/${encodeURIComponent(groupId)}`, { credentials: 'include' }),
      fetch(`/api/me/group-chats/${encodeURIComponent(groupId)}/messages`, { credentials: 'include' }),
    ])
      .then(async ([groupRes, msgsRes]) => {
        if (cancelled) return;
        if (!groupRes.ok) {
          if (groupRes.status === 403) router.push('/messages');
          return;
        }
        const groupData = await groupRes.json();
        const msgsData = msgsRes.ok ? await msgsRes.json() : { messages: [] };
        setGroupName(groupData.name ?? 'Группа');
        setParticipants(groupData.participants ?? []);
        setMessages((msgsData.messages ?? []).map((m: Partial<MessageItem>) => ({
          id: m.id!,
          senderId: m.senderId!,
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if ((!body && !attachment) || sending || !groupId) return;
    setSending(true);
    ...
    setReplyingTo(null);
    try {
      const res = await fetch(`/api/me/group-chats/${encodeURIComponent(groupId)}/messages`, {
        method: 'POST',
  ...
  async function handleVideoRecord() {
    ...
    try {
      ...
      const mime = ['video/webm;codecs=vp9,opus', 'video/webm'].find((t) => MediaRecorder.isTypeSupported(t)) ?? 'video/webm';
      const recorder = new MediaRecorder(stream);
      const chunks: Blob[] = [];
      recorder.ondataavailable = (e) => { if (e.data.size > 0) chunks.push(e.data); };
      recorder.onstop = async () => {
        ...
        const file = new File([blob], 'video.webm', { type: mime });
        const fd = new FormData();
        fd.append('file', file);
        try {
          const res = await fetch('/api/me/chat-upload', { method: 'POST', credentials: 'include', body: fd });
  ...
  async function handleFileSelect(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const res = await fetch('/api/me/chat-upload', { method: 'POST', credentials: 'include', body: fd });
      const data = await res.json();
      if (data.url && ['image', 'video', 'audio', 'file'].includes(data.type)) {
        setAttachment({ url: data.url, type: data.type, filename: data.filename || file.name });
      }
    } catch { /* ignore */ }
    e.target.value = '';
  }
  ...
  const searchForInvite = useCallback(() => {
    ...
    fetch(`/api/me/contacts/search?query=${encodeURIComponent(q)}`, { credentials: 'include' })
      ...
      .then((arr) => {
        ...
        setInviteResults(Array.isArray(arr) ? arr.filter((u: { id: string }) => !participantIds.has(u.id)) : []);
      })
      .catch(() => setInviteResults([]));
  }, [inviteQuery, participants]);

  useEffect(() => {
    const t = setTimeout(searchForInvite, 300);
    return () => clearTimeout(t);
  }, [inviteQuery, searchForInvite]);

  async function handleInvite(userId: string) {
    try {
      const res = await fetch(`/api/me/group-chats/${encodeURIComponent(groupId)}/participants`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16, paddingBottom: 12, borderBottom: '1px solid var(--border-subtle)', flexShrink: 0 }}>
        ...
        <div style={{ position: 'relative' }}>
          <button
            ...
            style={{ padding: '6px 12px', borderRadius: 8, border: '1px solid var(--border-color)', background: 'var(--bg-primary)', color: 'var(--text-primary)', cursor: 'pointer', fontSize: '0.9rem' }}
          >
          ...
          {showInvite && (
            <div style={{
              ...
              <div style={{ maxHeight: 200, overflowY: 'auto' }}>
                {inviteResults.map((u) => (
                  <div key={u.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 0', borderBottom: '1px solid var(--border-subtle)' }}>
                    {u.image ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img src={u.image} alt="" width={32} height={32} style={{ borderRadius: 8, objectFit: 'cover' }} />
                    ) : (
                      <div style={{ width: 32, height: 32, borderRadius: 8, background: 'var(--bg-accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.9rem', fontWeight: 600 }}>
                        {(u.name ?? u.id).slice(0, 1).toUpperCase()}
                      </div>
                    )}
                    <span style={{ flex: 1, fontSize: '0.9rem' }}>{u.name ?? `@${u.userTag ?? u.id.slice(0, 8)}`}</span>
                    <button
                      type="button"
                ...
                {inviteQuery.length >= 2 && inviteResults.length === 0 && (
                  <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', padding: 8 }}>Никого не найдено</p>
                )}
              </div>
            </div>
          )}
        ...
        <>
          <div ref={listRef} style={{ flex: 1, minHeight: 0, overflowY: 'auto', overflowX: 'hidden', padding: 16, display: 'flex', flexDirection: 'column', gap: 12, background: 'var(--bg-secondary)', borderRadius: 'var(--radius-md)', marginBottom: 16 }}>
            ...
            {messages.map((m) => {
              ...
              const displayName = participants.find((p) => p.userId === m.senderId)?.name ?? m.senderName ?? 'Участник';
              return (
                <div
                  key={m.id}
                  style={{
                ...
                >
                  ...
                  {m.attachmentUrl && m.attachmentType === 'image' && (
                    <a href={m.attachmentUrl} target="_blank" rel="noopener noreferrer" style={{ display: 'block', marginBottom: m.body && m.body !== '📎' ? 6 : 0 }}>
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img src={m.attachmentUrl} alt="" style={{ maxWidth: '100%', maxHeight: 240, borderRadius: 8 }} />
                    </a>
                  )}
                  ...
                  {m.attachmentUrl && m.attachmentType === 'file' && (() => {
                    ...
                    const displayName = nameMatch ? decodeURIComponent(nameMatch[1]) : 'Скачать файл';
                    return (
                      <a href={m.attachmentUrl.split('?')[0]} download target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '8px 12px', background: 'rgba(255,255,255,0.15)', borderRadius: 8, marginBottom: m.body && m.body !== '📎' ? 6 : 0, color: 'inherit', textDecoration: 'none', fontSize: '0.9rem' }}>
          ...
          {videoRecording && (
            <div style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'rgba(0,0,0,0.85)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 24 }}>
              <div style={{ width: 220, height: 220, borderRadius: '50%', overflow: 'hidden', border: '4px solid var(--accent-primary)' }}>
                <video ref={videoPreviewRef} muted playsInline autoPlay style={{ width: '100%', height: '100%', objectFit: 'cover', transform: 'scaleX(-1)' }} />
              </div>
              <p style={{ color: 'white', fontSize: '0.95rem' }}>Идёт запись видеокружка</p>
              <button type="button" onClick={handleVideoRecord} style={{ padding: '12px 24px', borderRadius: 24, border: 'none', background: 'var(--accent-red, #e53e3e)', color: 'white', fontWeight: 600, cursor: 'pointer' }}>
                Остановить
              </button>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} style={{ flexShrink: 0 }}>
            {attachment && (
              <div style={{ marginBottom: 8, position: 'relative', display: 'inline-flex', alignItems: 'center', gap: 8, padding: 8, background: 'var(--bg-accent)', borderRadius: 8 }}>
                {attachment.type === 'image' ? (
                  ...
                  <><span style={{ fontSize: '1.2rem', color: 'var(--accent-primary)' }}><i className="fa-solid fa-microphone" /></span><audio src={attachment.url} controls style={{ maxWidth: 200, height: 32 }} /></>
                ) : (
                  <span style={{ fontSize: '1.5rem', color: 'var(--accent-primary)' }}><i className="fa-solid fa-file-zipper" /></span>
                )}
                {attachment.type === 'file' && <span style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', maxWidth: 180, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{attachment.filename || 'Файл'}</span>}
            ...
            <div style={{ position: 'relative' }}>
              ...
              <button type="submit" disabled={sending || (!inputValue.trim() && !attachment)} className="platform-btn platform-btn-sm" style={{ alignSelf: 'stretch' }}>
                {sending ? '…' : 'Отправить'}
              </button>
            </div>
          </form>
